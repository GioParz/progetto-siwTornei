...
INSERT INTO partita (id, stato, goals_home, goals_away, arbitro_id, squadra_casa_id, squadra_ospite_id, torneo_id) VALUES (nextval('partita_seq'), 'TERMINATA', 2, 1, (SELECT id FROM arbitro WHERE cognome = 'Collina'), (SELECT id FROM squadra WHERE nome = 'Raptors FC'), (SELECT id FROM squadra WHERE nome = 'Titans Squad'), (SELECT id FROM torneo WHERE nome = 'Champions League'));

INSERT INTO partita (id, stato, goals_home, goals_away, arbitro_id, squadra_casa_id, squadra_ospite_id, torneo_id) VALUES (nextval('partita_seq'), 'PROGRAMMATA', 0, 0, (SELECT id FROM arbitro WHERE cognome = 'Rizzoli'), (SELECT id FROM squadra WHERE nome = 'Titans Squad'), (SELECT id FROM squadra WHERE nome = 'Spartans FC'), (SELECT id FROM torneo WHERE nome = 'Champions League'));